...
/**
 * 渠道属性字段
 * 这是一个扩展字段，用于存储各渠道特有的属性
 */
export const CHANNEL_ATTRIBUTES_FIELD: FieldDefinition = {
  key: 'customAttributes',
  label: '渠道属性',
  type: 'array',
  description: '渠道特有的属性，如品牌、UPC、MPN等',
};
...
export function getBasicFieldKeys(): string[] {
  return BASIC_FIELDS.map(f => f.key);
}

/**
 * 检查是否为基础字段
 */
export function isBasicField(key: string): boolean {
  return BASIC_FIELDS.some(f => f.key === key);
}
 ...
 * 获取存储字段（排除计算字段）
 */
export function getStorageFields(): FieldDefinition[] {
  return BASIC_FIELDS.filter(f => !f.computed);
}
