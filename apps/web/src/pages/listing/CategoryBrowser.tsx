...
const AUTO_GENERATE_RULES: Record<string, { name: string; description: string }> = {
  ...
  seat_material_extract: { name: '智能提取座面材质', description: '提取座面/承坐区域材质，如Fabric、Leather、Mesh等，有座面商品默认Conventional materials，无座面留空' },
  ...
  bed_size_extract: { name: '智能提取床尺寸', description: '从标题/描述提取北美标准床尺寸，如King/Queen/Full/Twin等，无则留空' },
  number_of_drawers_extract: { name: '智能提取抽屉数量', description: '从描述提取家具抽屉数量，支持数字和文字（如two drawers），默认0' },
  number_of_shelves_extract: { name: '智能提取搁板数量', description: '从描述提取家具搁板/层板数量，支持tier/level等表述，默认0' },
  theme_extract: { name: '智能提取主题', description: '从描述提取产品主题/风格设定，如Modern Luxury、Minimalist、Farmhouse等，默认Casual Home Comfort Setting' },
  shape_extract: { name: '智能提取形状', description: '从描述提取产品物理形状，如Round、Oval、Square、L-Shaped等，默认Rectangular' },
  diameter_extract: { name: '智能提取直径', description: '从描述提取产品直径尺寸，返回{unit, measure}格式，无则留空' },
  bed_style_extract: { name: '智能提取床风格', description: '从描述提取床的设计风格，如Upholstered Bed、Platform Bed、Storage Bed等，非床产品留空' },
  mount_type_extract: { name: '智能提取安装类型', description: '从描述提取产品安装方式，如Wall Mount、Ceiling Mount、Freestanding等，默认Freestanding' },
  fabric_content_extract: { name: '智能提取面料成分', description: '从描述提取软包/布艺家具的面料成分，支持百分比解析如"70% polyester / 30% cotton"，返回数组格式，无则留空' },
  configuration_extract: { name: '智能提取配置信息', description: '提取家具配置差异信息，包括结构形态、组合形式、尺寸级别、功能属性、座位容量等，用于区分同款式不同SKU，无则留空' },
  fabric_color_extract: { name: '智能提取布艺颜色', description: '仅提取明确与布艺/织物相关的颜色，如"Gray Fabric"、"Blue Velvet"，排除木材/金属等非织物颜色，无则留空' },
  accent_color_extract: { name: '智能提取装饰色', description: '提取装饰性次要颜色，如腿部、框架、五金等部件颜色，如"Black Legs"、"Gold Accents"，无则留空' },
  cushion_color_extract: { name: '智能提取坐垫颜色', description: '仅提取明确与坐垫/靠垫相关的颜色，如"Gray Cushions"、"Blue Throw Pillows"，无则留空' },
  number_of_panels_extract: { name: '智能提取面板数量', description: '仅适用于屏风/折叠屏/壁炉屏等产品，提取面板数量如"3-Panel Room Divider"，非屏风类产品留空' },
  seat_back_style_extract: { name: '智能提取靠背样式', description: '提取座椅靠背样式，如Tufted、Wingback、Ladder Back、High Back等，无明确样式留空' },
  power_type_extract: { name: '智能提取供电类型', description: '提取电动家具供电方式，如Electric、Battery Powered、USB Powered、Manual等，无则留空' },
  is_powered_extract: { name: '智能提取是否供电', description: '判断商品是否需要电力，返回Yes/No，默认No' },
  recommended_uses_extract: { name: '智能提取使用场景', description: '提取推荐使用场景，如Living Room、Bedroom、Outdoor、Home Office等，返回数组格式' },
  recommended_rooms_extract: { name: '智能提取推荐房间', description: '三层兜底策略：1.显式房间提取 2.商品类型默认映射 3.通用兜底值，确保必有值，返回数组格式' },
  mattress_firmness_extract: { name: '智能提取床垫硬度', description: '仅适用于床垫类商品，提取硬度等级（Plush/Medium/Firm等），支持数值映射，床垫类默认Medium，非床垫留空' },
  mattress_thickness_extract: { name: '智能提取床垫厚度', description: '仅适用于床垫类商品，提取厚度数值，支持英寸/厘米换算及Profile描述映射，返回{unit:"in",measure}格式，无明确信息则留空' },
  pump_included_extract: { name: '智能提取是否含气泵', description: '仅适用于充气床垫，判断是否包含气泵，返回Yes/No，充气床垫默认No，非充气床垫返回No' },
  fill_material_extract: { name: '智能提取填充材料', description: '提取坐垫/靠垫/床垫等填充材料，如Memory Foam、Polyester Fiber等，软体家具默认Foam/Polyester Fiber，无填充结构留空' },
  frame_material_extract: { name: '智能提取框架材料', description: '提取框架/支撑结构材料，如Solid Wood、Metal、Steel等，有框架商品默认Conventional materials，无框架留空' },
  table_height_extract: { name: '智能提取桌子高度', description: '仅适用于桌类商品，提取桌面至地面高度，支持英寸/厘米，返回{unit,measure}格式，无明确信息留空' },
  top_material_extract: { name: '智能提取顶部材质', description: '提取桌面/台面材质，如Glass、Marble、Wood等，有顶部商品默认Conventional materials，无顶部留空' },
  top_dimensions_extract: { name: '智能提取顶部尺寸', description: '提取桌面/台面尺寸（长×宽），支持英寸/厘米，返回"L x W in"格式，有顶部商品默认Standard Top Size' },
  top_finish_extract: { name: '智能提取顶部表面处理', description: '提取桌面表面处理，如Matte、Glossy、Distressed等，有顶部商品默认Natural Finish，无顶部留空' },
  hardware_finish_extract: { name: '智能提取五金表面处理', description: '提取五金部件表面处理，如Brushed Nickel、Chrome、Bronze等，有五金商品默认Standard Finish，无五金留空' },
  base_material_extract: { name: '智能提取底座材质', description: '提取底座/基座材质，如Metal、Wood、Stone等，有底座商品默认Standard Material，无底座留空' },
  base_color_extract: { name: '智能提取底座颜色', description: '提取底座/基座颜色，有底座商品默认Standard Color，无底座留空' },
  base_finish_extract: { name: '智能提取底座表面处理', description: '提取底座表面处理，如Matte、Polished等，有底座商品默认Standard Finish，无底座留空' },
  door_opening_style_extract: { name: '智能提取门开启方式', description: '提取门开启方式，如Swing、Sliding、Bi-Fold等，有门商品默认Standard，无门留空' },
  door_style_extract: { name: '智能提取门板样式', description: '提取门板外观样式，如Shaker、Slab、Glass等，有门商品默认Standard Door Style，无门留空' },
  slat_width_extract: { name: '智能提取板条宽度', description: '仅适用于有板条结构的商品，提取板条宽度，返回{unit,measure}格式，无明确信息留空' },
  number_of_hooks_extract: { name: '智能提取挂钩数量', description: '仅适用于挂钩类商品，提取挂钩数量，有挂钩但无明确数量默认1，无挂钩留空' },
  headboard_style_extract: { name: '智能提取床头板样式', description: '提取床头板样式，如Tufted、Panel、Upholstered等，有床头板默认Standard Headboard，无床头板留空' },
  frame_color_extract: { name: '智能提取框架颜色', description: '提取框架/支撑结构颜色，有框架商品默认Standard Color，无框架留空' },
  is_smart_extract: { name: '智能提取是否智能', description: '判断是否智能家具（含WiFi/蓝牙/App控制/语音助手等），返回Yes/No，强默认No' },
  is_antique_extract: { name: '智能提取是否古董', description: '判断是否真正古董（100年以上/明确标注Antique），返回Yes/No，强默认No，排除仿古风格' },
  is_foldable_extract: { name: '智能提取是否可折叠', description: '判断是否可折叠家具（Foldable/Folding/Collapsible），返回Yes/No，强默认No' },
  is_inflatable_extract: { name: '智能提取是否充气', description: '判断是否充气结构（Inflatable/Air Mattress/Blow Up），返回Yes/No，强默认No' },
  is_wheeled_extract: { name: '智能提取是否带轮', description: '判断是否带轮子（Wheeled/Casters/Rolling），返回Yes/No，强默认No' },
  is_industrial_extract: { name: '智能提取是否工业用途', description: '判断是否工业/商业用途（Industrial Grade/Commercial Use/Heavy-Duty），返回Yes/No，强默认No，排除纯工业风格' },
  assembled_product_length_extract: { name: '智能提取组装后长度', description: '优先从productLength获取，其次从文本提取，返回{unit,measure}格式，兜底1 in' },
  assembled_product_width_extract: { name: '智能提取组装后宽度', description: '优先从productWidth获取，其次从文本提取，返回{unit,measure}格式，兜底1 in' },
  assembled_product_height_extract: { name: '智能提取组装后高度', description: '优先从productHeight获取，其次从文本提取，返回{unit,measure}格式，兜底1 in' },
  assembled_product_weight_extract: { name: '智能提取组装后重量', description: '优先从productWeight获取，其次从文本提取，返回{unit,measure}格式（lb），兜底1 lb' },
...
const CHANNEL_DATA_OPTIONS = [
  ...
  { value: 'productDescription', label: '产品说明 (productDescription)' },
  { value: 'productCertification', label: '产品资质 (productCertification)' },
];
