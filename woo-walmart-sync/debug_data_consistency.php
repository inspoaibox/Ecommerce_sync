...
echo "0. 检查数据库表:\n";
$tables_exist = [];
$tables_to_check = [$upc_table, $batch_items_table];
...
if (!$tables_exist[$upc_table]) {
    echo "  ❌ UPC池表不存在，无法继续分析\n";
    exit;
}

if (!$tables_exist[$batch_items_table]) {
    echo "  ❌ 批量同步表不存在，无法继续分析\n";
    exit;
}
...
if (count($upc_assignments) > count($batch_products)) {
    ...
    if (!empty($duplicate_products)) {
        echo "可能原因: 产品重复分配UPC\n";
        echo "建议: 检查批量同步逻辑中是否有重复调用UPC分配的地方\n";
    } else {
